    ...
    def run(self, episodes=800):
        for ep in range(episodes):
            ...
            if ep % 10 == 0:
                ...
                print(f"--- Evaluation Result (Ep {ep}, Step {self.env_count}) ---")
                print(f"Avg Reward over 10 episodes: {avg_eval_reward:.2f} (Duration: {eval_duration:.2f}s)")
                print("--------------------------------------------------")
                wandb.log({"Eval Reward": avg_eval_reward})

                # Save best model
                if avg_eval_reward > self.best_eval_reward:
                    self.best_eval_reward = avg_eval_reward
                    best_model_path = os.path.join(self.save_dir, f"LAB5_StudentID_task2_pong.pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epsilon-decay", type=float, default=0.9999992)
