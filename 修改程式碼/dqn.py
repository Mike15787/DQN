...
class DQN(nn.Module):
    ...
    def __init__(self, input_shape, num_actions):
        ...
        if len(input_shape) == 1: # CartPole state (1D)
            print(f"Initializing MLP for input shape {input_shape}")
        ...
        elif len(input_shape) == 3: # Atari state (CxHxW)
            print(f"Initializing CNN for input shape {input_shape}")
            ...
            self.network = nn.Sequential(
                ...
                nn.Flatten(),
                nn.Linear(64 * 7 * 7, 512), nn.ReLU(), # 3136 input features
                nn.Linear(512, num_actions)
            )

    def forward(self, x):
        # Normalize Atari frames (uint8 -> float / 255.0)
        # CartPole inputs are float32 already
...
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        beta_annealing = self.beta_start + self.frame * (1.0 - self.beta_start) / self.beta_frames
        self.frame += 1
        beta = min(1.0, beta_annealing)
...
class DQNAgent:
    def __init__(self, env_name="CartPole-v1", args=None):
        ...
        env_state_shape = self.env.observation_space.shape # get the shape of env this time
        ...
        if self.is_atari:
            ...
            print(f"Atari env. State shape after preprocessing: {self.state_shape}")
        else:
            self.state_shape = env_state_shape # CartPole uses original shape (e.g., (4,))
            print(f"cartpole env. State shape: {self.state_shape}")
        ...
        self.best_eval_reward = float('-inf') # 這行是什麼?
    ...
    def select_action(self, state, evaluation=False):
        ...
        State to tensor conversion logic is inlined here.
        """
        ...
        if random.random() < epsilon_threshold:
            ...
            if not self.is_atari:
                # For non-Atari (CartPole), ensure float32
...
if __name__ == "__main__":
    ...
    task_defaults = {
        ...
        "Task2": {
            "env_name": "ALE/Pong-v5", "lr": 5e-5, "batch_size": 32, "memory_size": 200_000,
            "target_update_frequency": 1000, "replay_start_size": 10_000, "epsilon_min": 0.1,
            "epsilon_decay_steps": 1_000_000, "max_episode_steps": 10000, "train_per_env_step": 4, 
